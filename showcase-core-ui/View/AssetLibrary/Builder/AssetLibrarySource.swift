...
public struct AssetLibrarySourcesKey: EnvironmentKey {
  public static var defaultValue: [AssetLoader.SourceData] = []
}

public extension EnvironmentValues {
  var assetLibrarySources: AssetLibrarySourcesKey.Value {
  ...
  @ViewBuilder private func sections(_ groups: [String]) -> some View {
    ForEach(groups, id: \.self) { group in
      ...
      AssetLibrarySection(title(group)) {
        destination().assetLibrary(sources: sources)
      } preview: {
        preview().assetLibrary(sources: sources)
      } accessory: {
        accessory().assetLibrary(sources: sources)
      }
    }
  }
